package com.cp.im.utils.aliyun;

import cn.hutool.core.util.StrUtil;
import com.aliyun.oss.*;
import com.aliyun.oss.model.*;
import com.cp.im.config.AliYunConf;
...
public class AliYunOssClient {
    ...
    public static OSS getOSSClient() {
        return new OSSClientBuilder().build(AliYunConf.endpoint, AliYunConf.getAccessKeyId(), AliYunConf.getAccessKeySecret());
    }
     ...
     * @return
     */
    public static OSS getSHOSSClient(){
        return new OSSClientBuilder().build("http://oss-cn-shanghai.aliyuncs.com", AliYunConf.getAccessKeyId(), AliYunConf.getAccessKeySecret());
    }
...
//            ossClient.setBucketAcl(bucketName,CannedAccessControlList.PublicRead); // 存储空间（Bucket） 公共读，私有写
            ...
            PutObjectRequest putObjectRequest = new PutObjectRequest(AliYunConf.bucketName, fileName, new ByteArrayInputStream(file.getBytes()));
...
//            Date expiration = new Date(new Date().getTime() + 3600l * 1000 * 24 * 365 * 1);// 1年
            ...
            return AliYunConf.fileUrl + "/" + fileName;
    ...
    public static String uploadFile(String fileUrl, String fileName, String ext) {
        ...
        String aliUrl;
        // 构造客户端实例
        if (fileName.startsWith("userCard")){
            ossClient = getSHOSSClient();
            bucketName = "uploadcard";
            aliUrl = "https://uploadcard.oss-cn-shanghai.aliyuncs.com";
        }else {
            ossClient = getOSSClient();
            bucketName = AliYunConf.bucketName;
            aliUrl = AliYunConf.fileUrl;
        }
        try {
            ObjectMetadata metadata = new ObjectMetadata();
            String getcontentType = getcontentType(ext);
    ...
    public static InputStream gainFile(String fileName) {
        // 创建OSSClient实例
//        OSS ossClient = getOSSClient();
        OSS ossClient = new OSSClientBuilder().build(AliYunConf.endpoint, AliYunConf.getAccessKeyId(), AliYunConf.getAccessKeySecret());
//        // ossObject包含文件所在的存储空间名称、文件名称、文件元信息以及一个输入流。
        OSSObject ossObject = ossClient.getObject(AliYunConf.bucketName, fileName);
...
//        }
    ...
    public static void gainFileImg(String fileName, String width) {
        ...
        if (StrUtil.isNotEmpty(width)) {
            ...
            GetObjectRequest request = new GetObjectRequest(AliYunConf.bucketName, "");
    ...
    public static String createFolder(String folder) {
        ...
        try {
            ...
            // 判断文件夹是否存在, 不存在则创建
            if (!ossClient.doesObjectExist(AliYunConf.bucketName, keySuffixWithSlash)) {
                //创建文件夹
                ossClient.putObject(AliYunConf.bucketName, keySuffixWithSlash, new ByteArrayInputStream(new byte[0]));
                log.info("创建文件夹成功");
                //得到文件夹名
                OSSObject object = ossClient.getObject(AliYunConf.bucketName, keySuffixWithSlash);
    ...
    public static void delFile(String objectName) {
        ...
        try {
            ...
            ossClient.deleteObject(AliYunConf.bucketName, objectName);
        } catch (ClientException ce) {
    ...
    public static void delPrefixFile(String prefix) {
        ...
        do {
            ListObjectsRequest listObjectsRequest = new ListObjectsRequest(AliYunConf.bucketName)
                    .withPrefix(prefix)
            ...
            if (objectListing.getObjectSummaries().size() > 0) {
                ...
                DeleteObjectsRequest deleteObjectsRequest = new DeleteObjectsRequest(AliYunConf.bucketName)
                        .withKeys(keys);
